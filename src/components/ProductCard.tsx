...
const ProductCard = ({ price, title, description, delay = 0 }: ProductCardProps) => {
    return (
        <motion.div
            ...
            whileHover={{ y: -15 }}
            className="group relative w-full max-w-[450px] bg-white rounded-[3rem] p-12 transition-all duration-700 hover:shadow-premium border border-primary/5 hover:border-primary/20"
        >
            {/* Background Glow */}
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-start">
                    <h3 className="text-2xl font-bold text-gray-900 leading-tight tracking-tight uppercase">{title}</h3>
                    <div className="text-2xl font-bold text-primary leading-tight">₹{price}</div>
                </div>

                <p className="text-base text-gray-600 leading-relaxed pr-6 mb-6">
                    {description}
                </p>

                <div className="flex gap-4 pt-4">
                    <button className="flex-1 bg-primary text-white py-4 rounded-2xl font-medium text-base shadow-md hover:bg-black transition-all duration-300 hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 group-btn">
                        Order Now
