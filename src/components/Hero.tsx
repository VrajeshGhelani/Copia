...
const Hero = () => {
    ...
    return (
        <section
            ref={containerRef}
            className="relative min-h-screen pt-40 pb-24 px-6 flex items-center overflow-hidden transition-colors duration-700"
        >
            ...
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between relative z-10 w-full gap-16">
                ...
                <div className="flex-1 text-center md:text-left space-y-12 max-w-2xl px-4">
                    ...
                    <div className="space-y-6">
                        <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[1.1] text-gray-900 dark:text-white flex flex-wrap justify-center md:justify-start mb-6 drop-shadow-md uppercase italic transition-colors duration-700">
                            {text.map((char, i) => (
                                <motion.span
                                    key={i}
                                    ...
                                    className={char === " " ? "basis-full h-0" : `inline-block ${i >= 6 ? "text-primary" : "text-gray-900 dark:text-white"}`}
                                >
                                    {char === " " ? "" : char}
                                </motion.span>
                            ))}
                        ...
                        <motion.p
                            ...
                            className="text-base md:text-lg text-gray-700 dark:text-gray-300 leading-relaxed mb-4 max-w-xl md:mx-0 mx-auto transition-colors duration-700"
                        >
                    ...
                    >
                        <Link href="/contact">
                            <button className="bg-primary hover:bg-black text-white px-8 py-4 rounded-full font-medium text-lg shadow-premium transition-all duration-500 hover:scale-105 active:scale-95 group flex items-center gap-3 w-full sm:w-auto justify-center">
                                CONTACT NOW
                                <ArrowRight className="w-5 h-5 group-hover:translate-x-2 transition-transform" />
                            </button>
                        </Link>
                        <Link href="/products" className="text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white font-medium text-lg transition-colors border-b border-transparent hover:border-black dark:hover:border-white">
                            View Products
                        </Link>
                    </motion.div>
                </div>
